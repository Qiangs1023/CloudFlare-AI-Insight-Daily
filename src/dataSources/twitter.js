...
const TwitterDataSource = {
    async fetch(env, foloCookie) {
        ...
        for (let i = 0; i < fetchPages; i++) {
            ...
            try {
                console.log(`Fetching Twitter data, page ${i + 1}... with foloCookie: ${foloCookie ? 'present' : 'missing'}`);
                ...
                if (!response.ok) {
                    ...
                    console.error(`Failed to fetch Twitter data, page ${i + 1}: ${response.status} ${response.statusText}`, errorText);
                    break;
                }
                const data = await response.json();
                console.log(`Twitter API response for page ${i + 1}:`, JSON.stringify(data).substring(0, 500));
