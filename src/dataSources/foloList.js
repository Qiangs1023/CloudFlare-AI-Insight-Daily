...
const FoloListDataSource = {
    ...
    async fetch(env, foloCookie) {
        ...
        const fetchPages = parseInt(env.FOLO_LIST_FETCH_PAGES || '1', 10);
        const allFoloItems = [];
        ...
        if (!listId) {
            ...
            return {
                ...
                title: "Folo List Feeds",
                home_page_url: "https://app.folo.is",
                description: "Aggregated Folo list feeds",
                language: "zh-cn",
                items: []
            };
        }
        ...
        for (let i = 0; i < fetchPages; i++) {
            ...
            try {
                console.log(`Fetching Folo list data, page ${i + 1}...`);
                ...
                if (!response.ok) {
                    console.error(`Failed to fetch Folo list data, page ${i + 1}: ${response.statusText}`);
                    break;
                }
                const data = await response.json();
                if (data && data.data && data.data.length > 0) {
                    const filteredItems = data.data.filter(entry => isDateWithinLastDays(entry.entries.publishedAt, filterDays));
                    allFoloItems.push(...filteredItems.map(entry => ({
                        id: entry.entries.id,
                        ...
                        source: entry.feeds.title || 'Folo List',
                    })));
