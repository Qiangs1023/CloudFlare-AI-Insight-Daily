...
import { handleAutoGenerateDaily } from './handlers/autoGenerateDaily.js';
...
export default {
    async fetch(request, env) {
        ...
        if (path === '/login') {
            ...
            // 不需要认证，用于 GitHub Actions 自动抓取数据
            return await handleWriteData(request, env);
        } else if (path === '/autoGenerateDaily' && request.method === 'GET') {
            // 不需要认证，用于 GitHub Actions 自动生成日报
            return await handleAutoGenerateDaily(request, env);
        }
        ...
        try {
            if (path === '/getContentHtml' && request.method === 'GET') {
                ...
                response = await handleGetContentHtml(request, env);
